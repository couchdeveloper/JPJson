...
`JPAsyncJsonParser` provides an interface where a sequence of `NSData` buffers can 
 be sent via the message `parseBuffer:` to a `JPAsyncJsonParser` object. This makes 
 `JPAsyncJsonParser` especially suited to receive partial input which will be 
 ...
 <JPRepresentationGenerator> and <JPStreamSemanticActions>.
...
/*
 ...
 There can be many kinds of semantic actions. For instance, `JPRepresentationGenerator`
 ...
 objects, namely of `NSDictionary`s, `NSArray`s, and `NSNumber`s and `NSString`s. 
 ...
 *Designated Initializer*
 ...
 If object _semanticActions_ equals `nil` an instance of class `JPRepresentationGenerator`
 will be created and initialized with default properties. The default values 
 which will be set are documented in class <JPRepresentationGenerator>.

 If parameter _workerQueue_ equals `NULL` the parsing routines will be scheduled 
...
/**
 ...
 The content of _buffer_ may contain partial JSON text in which case subsequent 
 invocations of `parseBuffer:` will be required in order to finish parsing of
